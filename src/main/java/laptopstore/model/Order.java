...
import java.sql.SQLException; // Cần thiết nếu gọi ProductDataStore
import java.time.LocalDate;
import java.util.ArrayList;
import java.util.List;

import laptopstore.data.ProductDataStore; // Import ProductDataStore
...
public class Order {
    ...
    public void calculateAndSetTotals(ProductDataStore productDb) throws SQLException {
        ...
        this.netAmount = calculatedNetAmount.setScale(2, RoundingMode.HALF_UP);
        this.tax = this.netAmount.multiply(new BigDecimal("0.10")).setScale(2, RoundingMode.HALF_UP);
        this.totalAmount = this.netAmount.add(this.tax).setScale(2, RoundingMode.HALF_UP);
    }
    ...
    public String toString() {
        return "Order ID: " + orderId +
                (customerName != null ? " - Cust: " + customerName : (customerId != 0 ? " - Cust.ID: " + customerId : "")) +
